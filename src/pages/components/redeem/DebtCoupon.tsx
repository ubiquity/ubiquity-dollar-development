...
import * as widget from "src/pages/components/ui/widget";
import withLoadedContext, { LoadedContext } from "src/pages/components/lib/withLoadedContext";
import { formatTimeDiff, constrainNumber } from "src/pages/components/lib/utils";
import { useBalances, Balances, useTransactionLogger } from "src/pages/components/lib/hooks";
        ...
        <>
            <div className="my-8 inline-flex">
                ...
                <nav className="flex flex-col self-center border-b-0 sm:flex-row">
                    <button
                        className={`hover:text-accent m-0 self-center rounded-[16px] rounded-r-none focus:outline-none ${
                            selectedCurrency === uAR ? "border-accent text-accent font-medium" : "text-gray-600"
                        }`}
                    ...
                    <button
                        className={`hover:text-accent m-0 self-center rounded-[16px] rounded-l-none focus:outline-none ${
                            selectedCurrency === uDEBT ? "border-accent text-accent font-medium" : "text-gray-600"
                        }`}
        ...
        <>
                ...
                <div className="flex w-full">
                    ...
                    <div
                        className={`flex justify-${
                            leftPositioned ? "end border-right-accent border-r-2" : "center"
                        } border-0 border-r border-solid border-white/10`}
                        style={{ width: `${leftPositioned ? calculatedPercent() : 40}%` }}
                    >
                        {leftPositioned ? (
                            <span className="text-accent self-center pr-2 pt-0.5">${price}</span>
                        ) : (
                    ...
                    {leftPositioned ? (
                        <>
                            <div
                                className={`flex flex-col justify-center border-0 border-r border-solid border-white/10`}
                                style={{ width: `${40 - calculatedPercent()}%` }}
                            >
                                <svg
                                    xmlns="http://www.w3.org/2000/svg"
                                    className="text-accent ml-2 h-6 w-6"
                                    fill="none"
                                    viewBox="0 0 24 24"
                                    stroke="currentColor"
                                >
                        ...
                        <>
                            ...
                            >
                                <svg
                                    xmlns="http://www.w3.org/2000/svg"
                                    className="text-accent mr-2 h-6 w-6 self-end"
                                    fill="none"
                                    viewBox="0 0 24 24"
                                    stroke="currentColor"
                                >
                    ...
                    <div
                        className={`flex justify-${leftPositioned ? "center" : "start"}`}
                        style={{ width: `${leftPositioned ? 40 : 80 - calculatedPercent()}%` }}
                    >
                        {leftPositioned ? (
                            <span className="self-center pr-2">Pump cycle started {date} ago</span>
                        ) : (
                            <span className="text-accent self-center pl-2 pt-0.5">${price}</span>
                        )}
                    </div>
                    <hr className="m-0 h-full border-r-0 pt-1" />
            ...
            <div className="py-4">
                <span className="text-center">
                    {parseFloat(price) <= 1
                        ? "Burn uAD for debt coupons and help pump the price back up"
                        : "Time to redeem debts coupons and help move the price down"}
                </span>
            </div>
        </>
    );
