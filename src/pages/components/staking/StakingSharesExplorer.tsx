...
import { formatEther } from "src/pages/components/lib/format";
import { performTransaction, useAsyncInit } from "src/pages/components/lib/utils";
import withLoadedContext, { LoadedContext } from "src/pages/components/lib/withLoadedContext";
import { Container, Title, Icon, Loading, Button } from "src/pages/components/ui";
import { useBalances, useTransactionLogger } from "src/pages/components/lib/hooks";
...
export const StakingSharesExplorerContainer = ({ managedContracts, web3Provider, walletAddress, signer }: LoadedContext) => {
    ...
    async function fetchSharesInformation() {
        ...
        await Promise.all(
            stakingShareIds.map(async (id: { toString: () => string | number }): Promise<void> => {
...
export const StakingSharesInformation = ({ shares, totalShares, onWithdrawLp, onClaimUbq, onStake, processing, walletLpBalance }: Model & Actions) => {
    ...
    return (
        <div className="relative flex flex-col">
            ...
            <div className="border-accent/60 mb-6 rounded-lg border border-solid">
                <table className="m-0 w-full text-center">
                    <thead className="border-accent/60 border-b border-solid">
                        <tr>
            ...
            <div id="rewards-summary flex flex-col items-center justify-center">
                <div className="mb-2 flex items-center justify-center">
                    <Icon className="text-accent mr-2 w-4" icon="ubq" />
                    <span className="text-accent">{formatEther(totalPendingUgov)} </span>
                    &nbsp;pending UBQ rewards
                </div>
                <div className="mb-2 flex items-center justify-center">
                    <Icon className="text-accent mr-2 w-4" icon="liquidity" />
    ...
    return (
        <tr key={id} className="h-12" title={id.toString()}>
            ...
            <td className="border-r border-solid border-white/10">
                <div className="text-accent inline-flex items-center whitespace-nowrap">
                    <Icon icon="ubq" className="text-accent mr-2 w-4" /> <span>{formatEther(ugov)}</span>
                </div>
            </td>
