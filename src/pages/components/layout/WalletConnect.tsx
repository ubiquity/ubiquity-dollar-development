...
import { Container, Title, Button, Icon, Tooltip } from "src/pages/components/ui";
...
function Modal({ onClose, metamaskInstalled }: { onClose: () => void; metamaskInstalled: boolean }) {
    ...
    function Btn({ text, onClick, icon, iconSize = "md" }: { text: string; icon: string; iconSize?: string; onClick: () => void }) {
        return (
            <div onClick={() => onClick()} className="border-soli border-accent hover:bg-accent/20 cursor-pointer rounded-lg border p-4 text-center">
