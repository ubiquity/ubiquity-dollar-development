...
import { Icon, IconsNames } from "src/pages/components/ui";
...
const Sidebar = ({
    ...
    return (
        <>
            <div
                className={cx("border-r-accent/60 bg-paper fixed top-0 z-50 h-screen border-r border-solid transition-transform duration-500 ease-in-out", {
            ...
            >
                ...
                {state === "permanent" ? (
                    <Link href="/">
                        <a className="hover:text-accent hover:drop-shadow-light my-8 flex flex-col items-center justify-center uppercase tracking-widest transition duration-300 ease-in-out">
                ...
                {state === "hidden" || state === "hidden_hovering" ? (
                    <a
                        className={cx(
                            "border-accent/60 absolute top-[50%] left-full ml-[1px] flex items-center justify-center rounded-r border border-l-0 border-solid ",
                            {
...
const SocialLinkItem = ({ href, icon, alt }: { href: string; icon: IconsNames; alt: string }) => (
    <a
        href={href}
        className="hover:border-accent/0 hover:drop-shadow-light mx-1 flex h-10 w-10 items-center justify-center rounded-full border border-solid border-transparent p-2 text-white/75 transition duration-300 ease-in-out hover:bg-white/5 hover:transition-none"
        target="_blank"
        title={alt}
    >
                ...
                <a
                    className={cx(
                        "flex items-center justify-center text-ellipsis whitespace-nowrap rounded border border-solid py-2  pl-2 text-sm font-light uppercase tracking-widest transition duration-300 ease-in-out hover:transition-none",
                        {
                            ...
                            "hover:drop-shadow-light border-transparent text-white/75 hover:bg-white/5": !isActive,
                        }
                    )}
