...
import { Button } from "src/pages/components/ui";
...
const Stick = ({ amount, media, isConnected, loading }: { amount: number; media?: TokenData; isConnected: boolean; loading: boolean }) => {
    ...
    return (
        <div className="mx-4 max-w-sm">
            <div
                className={cx("relative mb-2 rounded-lg border-2 border-solid border-black border-opacity-25 shadow-inner", {
                    ["ring-accent ring-1"]: amount !== 0,
                })}
            >
                {isConnected && loading ? (
            ...
            <div className="text-accent drop-shadow-light mx-2 flex h-12 items-center text-opacity-75">
