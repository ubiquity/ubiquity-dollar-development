...
import { ZERO_ADDRESS } from "src/pages/components/lib/utils";
import { ERC20 } from "src/contracts/dollar/artifacts/types";
import { useManagerManaged, useWeb3 } from "src/pages/components/lib/hooks";
...
const useSimpleBond = (contracts: Contracts | null, tokensContracts: ERC20[]) => {
    ...
    async function refreshSimpleBondData() {
        if (provider && walletAddress && contracts && tokensContracts.length > 0 && ubqContracts) {
                ...
                await Promise.all(
                    tokensContracts.map((tokenContract) =>
                        Promise.all([
                            ...
                            newTokensRatios[tokenContract.address],
                        ])
                    )
                )
            ...
            const newRewardTokenBalance = +ethers.utils.formatUnits(
                await contracts.rewardToken.balanceOf(walletAddress),
                await contracts.rewardToken.decimals()
            );
