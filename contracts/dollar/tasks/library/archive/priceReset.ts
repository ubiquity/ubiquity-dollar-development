...
task("priceReset", "PriceReset can push uAD price lower or higher by burning LP token for uAD or 3CRV from the bonding contract")
  .addParam("amount", "The amount of uAD-3CRV LP token to be withdrawn")
  .addOptionalParam("pushhigher", "if false will withdraw 3CRV to push uAD price lower", true, types.boolean)
  .addOptionalParam("dryrun", "if false will use account 0 to execute price reset", true, types.boolean)
  .addOptionalParam("blockheight", "block height for the fork", 13135453, types.int)
  .setAction(
    async (
      taskArgs: {
        amount: string;
      ...
      const bondingMetapoolLPBalanceBefore = await metaPool.balanceOf(bondingAddr);
